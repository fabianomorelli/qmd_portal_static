...
engine.connect()
saida = pd.read_sql(sql, engine).sort_values("mes", ascending=True)
saida.rename(columns=months, inplace=True)
saida.set_index("mes", inplace=True)

data_graficos = pd.DataFrame()
data_graficos["Média de Focos"] = round(saida.groupby(saida.index).mean()["qtd"])
...
html = """
        <html><head><meta charset="UTF-8"></head>
            ...
            function drawChart(dados, tipo, container) {
              ...
              var options = {
                  title: 'Série histórica de focos por bioma e mês',
...
with open(
    os.path.join(path_saida, "grafico_serie_historica_estado_biomas_mes.html"), "w"
) as saida:
    saida.write(html)
