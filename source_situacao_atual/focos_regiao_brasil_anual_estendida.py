...
ARQUIVO_SAIDA = "%s/focos_regiao_brasil_anual_estendida.html"%(get.returnPath())
...
for key, value in regioes_dict.items():
    ...
    select 
       ...
       sum(nfocos) qtd 
    from 
       view_focos_munic_ref vre
    where 
        estado_id1 in ({lista_regioes}) and pais_id0=33
        and (
    ...
    """
    engine.connect()
    frames.append(pd.read_sql(sql, engine).sort_values(["ano"]))
